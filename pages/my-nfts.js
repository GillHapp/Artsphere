  ...
  useEffect(() => {
    fetchMyNFTsOrListedNFTs('fetchMyNFTs')
      .then((items) => {
        ...
        // Now filter for  NFTs by checking if the 'audio' field exists and is valid
        // const musicNFTs = items.filter((item) => item.image && typeof item.image === 'string' && item.image.trim() !== '');

        // console.log("Filtered  NFTs:", musicNFTs);
