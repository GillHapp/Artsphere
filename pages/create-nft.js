...
const CreateNFT = () => {
  ...
  const { isLoadingNFT, uploadImageToIPFS, createNFT } = useContext(NFTContext);
  ...
  //     // console.log("Uploaded  file URL:", url); // Add logging to check the result
  //     setFileUrl(url);
  //   } catch (error) {
  ...
  const fileStyle = useMemo(() => (
    ...
    ${coverDropzoneProps.isDragActive ? ' border-file-active' : ''} 
    ${coverDropzoneProps.isDragAccept ? ' border-file-accept' : ''} 
    ${coverDropzoneProps.isDragReject ? ' border-file-reject' : ''}`
  ), [coverDropzoneProps.isDragActive, coverDropzoneProps.isDragReject, coverDropzoneProps.isDragAccept]);
  ...
  const handleCreateNFT = async () => {
    ...
    if (!formInput.name || !formInput.description || !formInput.price || !coverUrl) {
          ...
          {/* <div className="mt-4">
            <div {...musicDropzoneProps.getRootProps()} className={fileStyle}>
              ...
              <div className="flexCenter flex-col text-center">
                ...
                <p className="font-poppins dark:text-white text-nft-black-1 font-semibold text-sm mt-2">Or browse  on your device</p>
              </div>
            </div>
            {fileUrl && (
              <aside>
