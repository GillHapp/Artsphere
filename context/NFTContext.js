...
export const NFTProvider = ({ children }) => {
  ...
  //     // console.log(' URL:', uploadedMusicUrl);
  ...
  //       method: 'POST',
  //       headers: {
  //         Authorization: 'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..EFv24JiIb3eZ3sFhVwtXrh1VKgBPqUVmteyErTcxvQE'
  //       },
  //       body: formData,
  //     });
  ...
  //     if (!response.ok) {
  //       throw new Error(`Failed to upload : ${result.error || 'Unknown error'}`);
  //     }
  ...
  // Create an NFT with  and Image Metadata
  const createNFT = async (formInput, router) => {
    const { name, description, price } = formInput;

    // Validate inputs
    if (!name || !description || !price || !uploadedImageUrl) {
    ...
    try {
      ...
      const metadata = {
        name,
        description,
        // audio: uploadedMusicUrl, // URL of the uploaded  file
  ...
  const buyNFT = async (nft) => {
    try {
      ...
      // console.log("Fetched metadata image URL:", image);

      const cid = image.split('/').pop(); // Extract CID from image URL
      ...
      if (!isPinned) {
        console.error('Image not pinned to Pinata. Aborting transaction.');
    ...
    try {
      ...
      const items = await Promise.all(data.map(async ({ tokenId, seller, owner, price: unformattedPrice }) => {
        try {
          ...
          const { data: { image, name, description } } = metadataResponse;
